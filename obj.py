...
#    def __repr__(self):
#        return "Socket object with: "+self._sock.getpeername()[0]+ str(self._sock.getpeername()[1])
#def delUser(dict, value):
#    for name, sock in dict.items():
#        if sock.getpeername() == value:
#            r = 1,sock.getpeername()[0], name
#            del dict[name]
#            return r
#    return 0, value[0]
list_connections = []
...
class Server():
    ...
    def auth(self, data, connection):
        ...
        if data['name'] in self.auth_users:
            print('Failed to login,existing user, ip=',connection.getpeername()[0],'user=',data['name'])
        ...
        while True:
            ...
            if self.validHeaders(headers):
                ...
                if msg == 'Successful login':
                    name = self.auth_users[act_addr]
                    print('Successful login from', act_addr, 'as', name)
                if msg == 'User exists':
                    break
                if msg == 'Disconnected by client':
                    break
            else:
                print("Disconnected: ", connection.getpeername(),'code=', 0)
